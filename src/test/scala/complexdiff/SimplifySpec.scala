...
class SimplifySpec extends FlatSpec with Matchers {
  ...
  "constants" should "be detected" in {
    Expr.isConst(Expr.const(7)) shouldBe true
    Expr.isConst(Sin(Z)) shouldBe false
    Expr.isConst(Sin(Expr.const(3))) shouldBe false
    Expr.getConst(Expr.const(4)) shouldEqual 4
    Expr.getConst(Expr.const(Complex.xy(5, 7))) shouldEqual Complex.xy(5, 7)
  }
  ...
  it should "compress nested exponents" in {
    Expr.pow(Expr.pow(Z, Expr.const(Complex.i*2)), Expr.const(3)).simplify() shouldEqual Term(ExprMap(Map(Z->6*Complex.i)))
  }
